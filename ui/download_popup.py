...
class DownloadPopup(QDialog):
    ...
    def _on_download_clicked(self):
        ...
        # 자막 받을 때만 언어 목록 포함
        self.result_data = OptionsUI(
            url=self.url,
            format=format_string,
            container=self.combo_format.currentText(),
            subtitle_only= self.checkbox_subtitle_only.isChecked(),
            subtitle_enabled=self.checkbox_subtitle.isChecked(),
            max_fragments=self.spin_fragments.value(),
            output_path=self.output_path,
            subtitle_langs=self._get_selected_subtitles() if self._get_selected_subtitles() else []
        )
