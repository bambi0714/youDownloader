...
class BrowserWindow(QMainWindow):
    ...
    #     main logic
    # ===================
    def _start_download(self, options:OptionsUI):
        import time
        start_time_dw = time.time()

        self.progress_dialog = ProgressDialog(self) # 프로그레스 바 UI
        self.download_thread = DownloadThread(options) #다운로드 스레드
        ...
        self.download_thread.start() # 다운로드 실행
        self.progress_dialog.exec()

        print(f"!!!! 다운로드 완료까지 {time.time() - start_time_dw:.2f}초 걸림")
